...
% 定义 VS Code 深色主题配色
\definecolor{vscode-bg}{rgb}{0.25,0.25,0.25} % 背景色
\definecolor{vscode-fg}{rgb}{0.8,0.8,0.8}   % 默认前景色
\definecolor{vscode-keyword}{rgb}{0.86,0.22,0.52} % 关键字颜色 (紫红色)
\definecolor{vscode-string}{rgb}{0.58,0.86,0.49}  % 字符串颜色 (绿色)
\definecolor{vscode-comment}{rgb}{0.58,0.86,0.49}    % 注释颜色 (绿色)
\definecolor{vscode-number}{rgb}{0.98,0.78,0.36}  % 数字颜色 (橙色)
\definecolor{vscode-function}{rgb}{0.49,0.63,0.85} % 函数名颜色 (蓝色)
\definecolor{vscode-type}{rgb}{0.86,0.58,0.36}    % 类型颜色 (棕色)
\definecolor{vscode-constant}{rgb}{1,1,1} % 常量颜色 
\definecolor{black}{rgb}{0,0,0} % 常量颜色-黑 


% 全局代码样式设置
\lstset{
    backgroundcolor=\color{vscode-bg},   % 背景颜色
    basicstyle=\ttfamily\small\color{vscode-fg}, % 基本字体样式
    keywordstyle=\color{vscode-keyword}\bfseries, % 关键字样式
    stringstyle=\color{vscode-string},  % 字符串样式
    commentstyle=\color{vscode-comment}\itshape, % 注释样式
    numberstyle=\color{vscode-constant}, % 行号样式
    numbers=right, 
    identifierstyle=\color{vscode-constant}, % 替代 constantstyle
    numbersep=5pt,                      % 行号与代码的间距
    frame=single,                       % 添加边框
    framesep=5pt,                       % 边框与代码的间距
    rulecolor=\color{vscode-bg},        % 边框颜色
    ...
    numbers=left,                       % 显示行号
    captionpos=b,                       % 标题位置
    morekeywords={def, class, import, from, as, if, else, elif, for, while, return, True, False, None}, % 自定义关键字
}
...
\newpage
\section*{摘要}
本文提出了一种基于融合式中文编码模型的文本编码器——PolicyBERT，旨在提升中文政策文本的语义理解与生成能力。通过引入HanLP分词、门控机制和多头注意力机制，PolicyBERT能够有效融合字符级和词级信息，增强对中文政策文本的建模能力。本文构建了两个适用于中文政策文本的下一句预测（NSP）任务数据集（PolicySM-1和PolicySM-2），并在这些数据集上进行了模型微调。实验结果表明，使用多头注意力机制的模型在NSP任务中表现最佳，达到了0.9325的准确率。此外，本文基于微调后的模型，使用Ollama与langchain-chatchat架构搭建了一个基于检索增强生成（RAG）机制的智能问答系统“未名问政”，实现了从文本检索到问答生成的全流程部署。该系统具备部署灵活、响应快速、扩展便捷等特点，适用于政务智能问答、政策知识库构建与自动解读等场景。本文的研究为中文政策文本处理提供了新的方法和工具，具有重要的理论意义和实际应用价值。

\vspace{2em} % 添加空行

\textbf{关键词：}PolicyBERT，中文政策文本，融合式编码模型，检索增强生成，智能问答系统，BERT
\vspace{2em} % 添加空行

\newpage
\section*{Abstract}
This paper proposes a fused Chinese encoding model-based text encoder—PolicyBERT—designed to enhance the semantic understanding and generation capabilities for Chinese policy documents. By incorporating HanLP-based tokenization, a gating mechanism, and multi-head attention, PolicyBERT effectively integrates character-level and word-level information, thereby improving its modeling ability for Chinese policy texts. Two next sentence prediction (NSP) datasets tailored for Chinese policy texts, namely PolicySM-1 and PolicySM-2, were constructed and used for model fine-tuning. Experimental results show that the model with multi-head attention achieves the best performance on the NSP task, reaching an accuracy of 0.9325. Furthermore, based on the fine-tuned model, this study builds an intelligent question-answering system named "WeiMingPolicyQA" using the Ollama and langchain-chatchat framework, leveraging the retrieval-augmented generation (RAG) mechanism to enable an end-to-end pipeline from document retrieval to answer generation. The system features flexible deployment, fast response, and easy scalability, making it suitable for applications such as intelligent governance Q\&A, policy knowledge base construction, and automatic policy interpretation. This research provides new methods and tools for processing Chinese policy documents, with significant theoretical and practical value.


\vspace{2em} % 添加空行

\textbf{Keywords:} PolicyBERT, Chinese policy text, fused encoding model, retrieval-augmented generation, intelligent question-answering system, BERT
...
\subsection{实验结果}
\subsubsection{公开数据集}
首先，我们将在公开数据集上进行实验，与现有的NLP领域文本编码器进行对比。包括 BERT-wwm, ERNIE 1.0,ERNIE 2.0, NEZHA 和 ZEN。我们使用了 ZEN的预训练模型作为基线模型，并在此基础上进行微调。我们使用了不同的融合方法（门控机制、多头注意力机制）与原始论文中直接相加进行对比。由于缺少大规模预训练语料，我们无法在公开数据集上应用Hanlp分词。我们在公开数据集上仅测试了不同融合方法与n-gram表示的效果。
...
\begin{itemize}
    ...
    \item \textbf{文档分类（DC）：} 使用 THUCNews 数据集（Sun 等, 2016），该数据集来源于新浪新闻，包含 10 个类别，类别分布均匀。
    \item \textbf{情感分析（SA）：} 使用 ChnSentiCorp（CSC）数据集，该数据集包含来自图书、计算机和酒店三个领域的 12,000 篇文档。
    \item \textbf{句子对匹配（SPM）：}使用 LCQMC 数据集（Liu 等, 2018）\cite{liu-etal-2018-lcqmc}，该数据集中的每一个样本都是一个句子对，和一个标签，表示这两个句子是否语义相似。
    \item \textbf{自然语言推理（NLI）：}使用 XNLI 自然语言推理语料库（Conneau 等，2018）\cite{conneau2018xnli}，包含 MultiNLI 语料库的 5000 个测试对和 2500 个开发对。
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{l|ccccc}
        \toprule
        \textbf{Task} & CWS & POS & NER & DC & SA \\
        \midrule
        \textbf{Dataset} & MSR & CTB5 & MSRA & THUCNews & CSC \\
        \midrule
        \textbf{Train} & 87K & 18K & 45K & 50K & 10K \\
        \textbf{Test} & 4K & 1K & 3K & 10K & 1K \\
        \bottomrule
    \end{tabular}
    \label{tab:public_datasets}
\end{table}

实验结果如表 \ref{tab:public_results} 所示。可以看到，使用门控机制和多头注意力机制的模型在所有任务上都取得了显著的性能提升

\begin{table}[H]
    \renewcommand{\arraystretch}{1}
    \centering
    \caption{公开数据集上不同模型的f1值对比}
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model\textbackslash Task} & CWS & POS & NER & DC & SA \\
        \midrule
        BERT      & 0.9720 & 0.9543 & 0.9312 & 0.9671 & 0.9410 \\
        \midrule
        BERT-wwm     & - & - & 0.9510 & 0.9760 & 0.9500\\
        ERNIE 1.0    & - & - & 0.9510 & 0.9730 & 0.9540\\
        ERNIE 2.0(B) & - & - & - & - & 0.9550 \\
        NEZHA(B)     & - & - & - & - & 0.9517 \\
        NEZHA-wwm(B) & - & - & - & - & 0.9584 \\
        \midrule
        ...
        NEZHA-wwm(L) & - & - & - & - & 0.9600 \\
        \midrule
        ZEN       & 0.9789 & 0.9582 & 0.9324 & 0.9687 & 0.9442 \\
        \midrule
        $PolicyBERT_{gate}$ & \textbf{0.9816} & \textbf{0.9716} & 0.9422          & \textbf{0.9655} & -\\ 
        $PolicyBERT_{attn}$ & 0.9812          & 0.9711          & \textbf{0.9437} & 0.9651 & -\\
        \bottomrule
    \end{tabular}
